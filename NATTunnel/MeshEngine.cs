...
public class MeshEngine
{
    ...
    private TimeSpan repairCooldown = TimeSpan.FromSeconds(TunnelOptions.RepairCooldownSeconds);
    ...
    private TimeSpan peerDiscoveryInterval = TimeSpan.FromSeconds(TunnelOptions.HeartbeatIntervalSeconds);
    ...
    private TimeSpan heartbeatInterval = TimeSpan.FromSeconds(TunnelOptions.HeartbeatIntervalSeconds);
    ...
    private int peerDeadThreshold = TunnelOptions.DeadThreshold; // Declare dead after N consecutive missed acks
    ...
    public void Run(WireGuardTunnel wireguardTunnel, string meshIP, UdpClient udpClient, WireGuardUdpProxy udpProxy, Guid peerID)
    {
        ...
        // and feeds the background listener that all peers depend on.
        try
        {
            meshControlClient = new UdpClient(MeshControlPort);
            Program.Log($"[Mesh] Mesh control listening on UDP port {MeshControlPort}");
        }
        ...
        try
        {
            ...
            while (!Program.ShutdownRequested)
            {
                ...
                if (isFirstIteration && !TunnelOptions.AutoConnect)
                {
                    ...
                    Program.ConnectionState = MeshConnectionState.Disconnected;
                    Program.Log("[Mesh] Idle (autoConnect=false). Waiting for /connect request...");
                    while (!Program.ShutdownRequested && !Program.ConnectRequested)
                        System.Threading.Thread.Sleep(100);
                    Program.ConnectRequested = false;
                    if (Program.ShutdownRequested) break;
                }
                isFirstIteration = false;

                Program.ConnectionState = MeshConnectionState.Connecting;
                Program.DisconnectRequested = false;
                {
                    int handshakeDelay = 5;
                    for (int attempt = 1; ; attempt++)
                    {
                        if (Program.ShutdownRequested) return;
                        if (Program.DisconnectRequested) break;
                        try
                        {
                            ...
                            if (!connected || Program.DisconnectRequested)
                            {
                                tcpClient.Close();
                                if (Program.DisconnectRequested) break;
                            ...
                            if (TunnelOptions.TlsEnabled)
                            {
                                ...
                                stream = sslStream;
                                Program.Log($"[Mesh] TLS handshake complete (protocol: {sslStream.SslProtocol})");
                            }
                            else
                            {
                            ...
                            if (natTypeResponse.ID == MediationMessageType.NATTypeResponse)
                            {
                                ...
                                Program.Log($"[Mesh] NAT type detected: {detectedNatType}");
                            }
                            ...
                            Program.Log($"[Mesh] Joined network! Found {joinResponse.PeerCount} other peers");
                            handshakeDelay = 5; // Reset on success
                            break;
                        }
                        catch (Exception ex) when (!Program.ShutdownRequested)
                        {
                            Program.Log($"[Mesh] Mediation handshake failed: {ex.Message}");
                            try { tcpClient?.Dispose(); } catch { }
                            tcpClient = null;
                            stream = null;
                            earlyTcpRemainder = "";
                            Program.Log($"[Mesh] Retrying in {handshakeDelay}s (attempt {attempt})...");
                            // Sleep in short intervals so DisconnectRequested is checked promptly
                            for (int ms = 0; ms < handshakeDelay * 1000 && !Program.DisconnectRequested && !Program.ShutdownRequested; ms += 100)
                                System.Threading.Thread.Sleep(100);
                ...
                if (Program.DisconnectRequested)
                {
                    Program.ConnectionState = MeshConnectionState.Disconnected;
                    try { tcpClient?.Dispose(); } catch { }
                    tcpClient = null; stream = null;
                    Program.Log("[Mesh] Disconnected during handshake — waiting for reconnect");
                    while (!Program.ShutdownRequested && !Program.ConnectRequested)
                        System.Threading.Thread.Sleep(100);
                    Program.ConnectRequested = false;
                ...
                Program.ConnectionState = MeshConnectionState.Connected;
                ...
                if (joinResponse.Peers != null)
                {
                    foreach (var peer in joinResponse.Peers)
                    {
                            ...
                            pid == joinResponse.IntroducerPeerID && !string.IsNullOrEmpty(mip))
                        {
                            ...
                            Program.Log($"[Mesh] Introducer mesh IP: {introducerMeshIP} (peer {pid})");
                        }
                    }
                    Program.Log($"[Mesh] Cached {peerInfoByMeshIP.Count} peer(s) from initial join response");
                }
                ...
                if (takenMeshIPs.Contains(meshIP))
                {
                    ...
                    {
                        Program.Log($"[Mesh] WARNING: Mesh IP collision detected ({originalMeshIP} already taken) and no free slot found in hash offsets. Keeping original IP — connectivity may be impaired.");
                    }
                }
                ...
                {
                    Program.Log("[Mesh] No other peers in network yet - waiting for others to join...");
                }

                // Keep connection alive and listen for ConnectionBegin messages
                Program.Log("[Mesh] Mesh networking active. Waiting for connections...");
                Program.Log("[Mesh] Press Ctrl+C to exit.");
                ...
                {
                    isIntroducer = true;
                    Program.Log("[Mesh] Server designated us as the introducer in join response");
                }
                else if (detectedNatType != NATType.Symmetric && joinResponse.Peers != null)
                {
                    ...
                    {
                        isIntroducer = true;
                        Program.Log("[Mesh] We're the only non-symmetric peer — staying connected as potential introducer");
                    }
                }
                ...
                if (!udpDispatcherStarted)
                {
                    ...
                    System.Threading.Tasks.Task.Run(() =>
                    {
                        ...
                        Program.Log("[Mesh] Shared UDP dispatcher started");
                        while (true)
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    foreach (var tunnel in tunnels)
                                    {
                                        ...
                                        {
                                            Program.Log($"[Mesh] Error dispatching packet to tunnel: {ex.Message}");
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                Program.Log($"[Mesh] UDP dispatcher error: {ex.Message}");
                            }
                        }
                    });
                ...
                Program.meshStateProvider = GetMeshState;
                ...
                while (!Program.ShutdownRequested && !Program.DisconnectRequested)
                {
                    ...
                    if (!isIntroducer && tcpClient.Connected && hasPeers)
                    {
                        ...
                        if (readyToDisconnect && disconnectAfter == null)
                        {
                            ...
                            Program.Log($"[Mesh] All initial connections established — grace period started ({gracePeriod}s)");
                        }
                        else if (!readyToDisconnect && disconnectAfter != null)
                        {
                            // New connection arrived during grace period — reset timer
                            disconnectAfter = null;
                            Program.Log("[Mesh] New connection activity — grace period reset");
                        }
                        else if (readyToDisconnect && disconnectAfter != null && DateTime.UtcNow > disconnectAfter.Value)
                        {
                            Program.Log("[Mesh] Grace period elapsed — disconnecting from mediation server");
                            tcpClient.Close();
                            break;
                        }
                    }
                    ...
                    {
                        if (isIntroducer)
                            Program.Log("[Mesh] Mediation server connection lost — introducer role ended");
                        else
                            Program.Log("[Mesh] TCP connection to mediation server lost during setup");
                        break;
                    }
                    ...
                    if (pendingConnectionRequests.Count > 0)
                    {
                        ...
                        foreach (var staleID in staleRequests)
                        {
                            ...
                            Program.Log($"[Mesh] Removed stale pending connection request for {staleID} (no response in {staleTimeout.TotalSeconds}s)");
                        }
                    }
                    ...
                    if (DateTime.UtcNow - lastKeepAlive > keepAliveInterval)
                    {
                        ...
                        {
                            Program.Log($"[Mesh] Keep-alive write failed, connection lost: {ex.Message}");
                            break;
                        }
                    }
                    ...
                    {
                        Program.Log("[Mesh] No active peers — sending periodic discovery request");
                        try
                        {
                        ...
                        {
                            Program.Log($"[Mesh] Discovery write failed, connection lost: {ex.Message}");
                            break;
                        }
                    }
                        ...
                        pendingTunnelCount == 0)
                    {
                        ...
                        {
                            Program.Log($"[Mesh] Introducer connection failed after {MaxIntroducerRetries} retries — disconnecting to force fresh mediation reconnection");
                            introducerRetryCount = 0;
                            try { tcpClient.Close(); } catch { }
                            break; // Break to mesh-control-only loop; isolation detection will reconnect
                        }

                        Program.Log($"[Mesh] Retrying connection to introducer {joinResponse.IntroducerPeerID} (attempt {introducerRetryCount}/{MaxIntroducerRetries})");
                        try
                        {
                        ...
                        {
                            Program.Log($"[Mesh] Introducer retry write failed: {ex.Message}");
                            break;
                        }
                    }
                         ...
                         (introducerMeshIP != null && activePeerTunnels.ContainsKey(introducerMeshIP))))
                    {
                        ...
                        {
                            Program.Log($"[Mesh] Introducer connection restored after {introducerRetryCount} retries");
                            introducerRetryCount = 0;
                        }
                    }
                        ...
                        DateTime.UtcNow - lastIntroducerProbe > introducerProbeInterval)
                    {
                        if (!introducerProbeAckReceived)
                        {
                            ...
                            Program.Log($"[Mesh] Introducer ({introducerMeshIP}) missed probe ack ({introducerMissedProbes}/{IntroducerMissedProbeThreshold})");
                        }
                        else
                        {
                            if (introducerMissedProbes > 0)
                                Program.Log($"[Mesh] Introducer ({introducerMeshIP}) responded — resetting missed probe count");
                        ...
                        if (introducerMissedProbes >= IntroducerMissedProbeThreshold)
                        {
                            ...
                            Program.Log($"[Mesh] Introducer confirmed dead (primary loop) — election delay {delayMs}ms");
                            for (int slept = 0; slept < delayMs; slept += 100)
                            {
                                System.Threading.Thread.Sleep(100);
                                if (Program.ShutdownRequested || Program.DisconnectRequested) break;
                                if (introducerMeshIP != electionTarget)
                                {
                                    Program.Log("[Mesh] Primary-loop election aborted — another peer became introducer during delay");
                                    introducerMissedProbes = 0;
                                    introducerProbeAckReceived = true;
                                    break;
                                }
                            }
                            if (introducerMeshIP == electionTarget && !Program.ShutdownRequested && !Program.DisconnectRequested)
                            {
                                Program.Log("[Mesh] Election delay elapsed — sending MeshJoinRequest, awaiting server's choice");
                                introducerMissedProbes = 0;
                                ...
                                {
                                    Program.Log($"[Mesh] Failed to send takeover MeshJoinRequest: {ex.Message}");
                                }
                            }
                        }
                        else
                        {
                            ...
                            catch (Exception)
                            {
                                ...
                                // we'll trigger takeover — no need to Program.Log per-attempt noise.
                            }
                        }
                    ...
                    try
                    {
                        ...
                        if (bytesRead == 0)
                        {
                            Program.Log("[Mesh] Mediation server closed connection");
                            break;
                        }
                    ...
                    if (tcpBuffer.Length > 0 && tcpBuffer.Contains('{'))
                    {
                        ...
                        while (jsonStartIndex < tcpBuffer.Length)
                        {
                            ...
                            {
                                Program.Log($"[Mesh] Could not parse JSON object: {parseEx.Message}");
                            ...
                            {
                                Program.Log($"[Mesh] Received connection request! ConnectionID: {msg.ConnectionID}, Endpoint: {msg.EndpointString}");
                                Program.Log($"[Mesh] Waiting for ConnectionBegin to establish tunnel...");
                            ...
                            {
                                Program.Log($"[Mesh] *** ConnectionBegin received! ***");
                                Program.Log($"[Mesh] ConnectionBegin: connID={msg.ConnectionID}, endpoint={msg.EndpointString}, NAT={msg.NATType}, meshIP={msg.PrivateAddressString}");
                                ...
                                if (!string.IsNullOrEmpty(msg.PrivateAddressString))
                                {
                                    ...
                                    foreach (var oldConnID in oldConnIDs)
                                    {
                                        ...
                                        if (oldTunnel != null)
                                        {
                                            Program.Log($"[Mesh] Disposing old tunnel {oldConnID} for {cbMeshIP} (superseded by {msg.ConnectionID})");
                                ...
                                {
                                    Program.Log($"[Mesh] Tunnel {msg.ConnectionID} already exists - ignoring duplicate ConnectionBegin");
                                }
                                else
                                {
                                    ...
                                    var peerTunnel = new Tunnel(
                                        onConnectionFailure: () =>
                                        {
                                            Program.Log($"[Mesh] Tunnel {capturedConnectionID} failed permanently after all retries — cleaning up for future retry");
                                            lock (meshLock)
                                            {
                                        ...
                                        {
                                            Program.Log($"[Mesh] Tunnel {capturedConnectionID} WireGuard connection established");
                                            System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
                                            ...
                                            lock (meshLock)
                                            {
                                                if (peerMeshIPs.TryGetValue(capturedConnectionID, out string completedMeshIP) && !string.IsNullOrEmpty(completedMeshIP))
                                                {
                                                    ...
                                                    {
                                                        Program.Log($"[Mesh] Flushing {deferred.Count} deferred MeshConnectionBegin message(s) for {completedMeshIP}");
                                                        foreach (var deferredMsg in deferred)
                                                        {
                                                            ...
                                                            try
                                                            {
                                                                ...
                                                                Program.Log($"[Mesh] Sent deferred MeshConnectionBegin to {targetIP}");
                                                            }
                                                            catch (Exception ex)
                                                            {
                                                                Program.Log($"[Mesh] Failed to send deferred MeshConnectionBegin to {targetIP}: {ex.Message}");
                                                            }
                                                        }
                                    ...
                                    System.Threading.Tasks.Task.Run(() =>
                                    {
                                        ...
                                        {
                                            Program.Log($"[Mesh] Error starting tunnel {capturedConnectionID}: {ex.Message}");
                                            System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
                                        }
                                    });
                                }
                            ...
                            else if (msg.ID == MediationMessageType.MeshJoinResponse)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(msg.IntroducerPeerID))
                                    {
                                        ...
                                        {
                                            Program.Log("[Mesh] Server confirmed us as new introducer (primary loop)");
                                            isIntroducer = true;
                                            introducerMeshIP = meshIP;
                                        }
                                        else if (!serverSaysWereIntroducer && isIntroducer)
                                        {
                                            Program.Log($"[Mesh] Server picked a different introducer ({msg.IntroducerPeerID}) — relinquishing role");
                                            isIntroducer = false;

                                            // Tear down all relay routes we were hosting as introducer.
                                            // The new introducer doesn't know about them and won't be able
                                            // to drive reselection, so just drop them and let the new
                                            // introducer re-establish on demand.
                                            ...
                                            if (droppedAtRelinquish.Length > 0)
                                                Program.Log($"[Mesh] Relinquished introducer: dropped {droppedAtRelinquish.Length} hosted relay pair(s)");
                                        }
                                        if (!serverSaysWereIntroducer && msg.Peers != null)
                                        {
                                            ...
                                            foreach (var peerObj in msg.Peers)
                                            {
                                                ...
                                                if (pid == msg.IntroducerPeerID && !string.IsNullOrEmpty(mip))
                                                {
                                                    ...
                                                    {
                                                        Program.Log($"[Mesh] Introducer updated to {mip} per server");
                                                        introducerMeshIP = mip;
                                                        introducerMissedProbes = 0;
                                                        introducerProbeAckReceived = true;
                                                    }
                            ...
                            {
                                Program.Log($"[Mesh] Updated peer list received: {msg.PeerCount} peers");
                            ...
                            {
                                Program.Log($"[Mesh] Received ConnectionComplete (routing to tunnel)");
                            ...
                            {
                                Program.Log($"[Mesh] ServerNotAvailable — target peer unavailable");
                            ...
                            else if (msg.ID == MediationMessageType.MeshIntroduceRequest)
                            {
                                ...
                                isIntroducer = true;
                                Program.Log($"[Mesh] Selected as introducer for new peer {msg.PeerID}");
                                ...
                                if (msg.OtherPeers != null)
                                {
                                    foreach (var peerObj in msg.OtherPeers)
                                    {
                                        ...
                                        {
                                            Program.Log($"[Mesh] Skipping peer with no mesh IP in OtherPeers list");
                                            continue;
                                        }
                                        ...
                                        // Same-NAT short-circuit: even two symmetric peers can reach each other
                                        // LAN-direct if they share a public IP and both have LAN endpoints. Skip
                                        // the symmetric-relay branch and let the normal hole-punch path below
                                        // substitute LAN endpoints.
                                        ...
                                        // Symmetric ↔ symmetric: hole-punch infeasible across NATs — pick a relay.
                                        // Exception: same-LAN pairs reach each other directly via local endpoints.
                                        if (!sameLan && msg.NATType == NATType.Symmetric && (NATType)existingPeerNatType == NATType.Symmetric)
                                        {
                                            string chosenRelay = PickRelay(existingPeerMeshIP, msg.PrivateAddressString) ?? (TunnelOptions.AllowRelayThrough ? meshIP : null);
                                            if (string.IsNullOrEmpty(chosenRelay))
                                            {
                                                Program.Log($"[Mesh] No eligible relay for {existingPeerMeshIP} <-> {msg.PrivateAddressString} and self-relay disabled — skipping pair");
                                                continue;
                                            }
                                            Program.Log($"[Mesh] Both {msg.PeerID} and {existingPeerID} are symmetric NAT — relay via {(chosenRelay == meshIP ? "self (introducer)" : chosenRelay)}");
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    Program.Log($"[Mesh] Failed to send MeshRelayAssignment to {chosenRelay}: {ex.Message}");
                                                }
                                            }
                                            ...
                                            {
                                                Program.Log($"[Mesh] Failed to send relay MeshConnectionBegin to {existingPeerMeshIP}: {ex.Message}");
                                            }
                                            ...
                                            if (!string.IsNullOrEmpty(msg.PrivateAddressString))
                                            {
                                                ...
                                                if (completedTunnelMeshIPs.Contains(msg.PrivateAddressString))
                                                {
                                                    ...
                                                    {
                                                        Program.Log($"[Mesh] Failed to send relay MeshConnectionBegin to {msg.PrivateAddressString}: {ex.Message}");
                                                    }
                                                }
                                                else
                                                {
                                            ...
                                            !string.IsNullOrEmpty(msg.LocalIP) && !string.IsNullOrEmpty(existingPeerLocalIP))
                                        {
                                            ...
                                            Program.Log($"[Mesh] Same-NAT detected! Using LAN endpoints: {newPeerEndpointForExisting} <-> {existingPeerEndpointForNew}");
                                        }
                                        ...
                                        if (tunnelToNewReady)
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                MeshSend(toExistingBytes, toExistingBytes.Length,
                                                    new IPEndPoint(IPAddress.Parse(existingPeerMeshIP), MeshControlPort));
                                                Program.Log($"[Mesh] Sent MeshConnectionBegin to existing peer {existingPeerMeshIP} (about new peer {msg.PeerID})");
                                            }
                                            catch (Exception ex)
                                            {
                                                Program.Log($"[Mesh] Failed to send MeshConnectionBegin to {existingPeerMeshIP}: {ex.Message}");
                                            }
                                            ...
                                            if (connBeginToNew != null)
                                            {
                                                try
                                                {
                                                    ...
                                                    MeshSend(toNewBytes, toNewBytes.Length,
                                                        new IPEndPoint(IPAddress.Parse(msg.PrivateAddressString), MeshControlPort));
                                                    Program.Log($"[Mesh] Sent MeshConnectionBegin to new peer {msg.PrivateAddressString} (about existing peer {existingPeerMeshIP})");
                                                }
                                                catch (Exception ex)
                                                {
                                                    Program.Log($"[Mesh] Failed to send MeshConnectionBegin to {msg.PrivateAddressString}: {ex.Message}");
                                                }
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            Program.Log($"[Mesh] Deferred MeshConnectionBegin for both peers (tunnel to {msg.PrivateAddressString} not yet established)");
                                        }
                                ...
                                try
                                {
                                    ...
                                    Program.Log($"[Mesh] Sent MeshIntroduceAck for {msg.PeerID} ({introduced} introduced, {deferredCount} deferred, completedTunnels={completedTunnelMeshIPs.Count})");
                                }
                                catch (Exception ex)
                                {
                                    Program.Log($"[Mesh] MeshIntroduceAck write failed, connection lost: {ex.Message}");
                                    tcpClient.Close();
                                    break;
                                }
                            }
                ...
                else
                {
                    ...
                    Program.Log("[Mesh] Entering mesh-control-only mode (fully disconnected from mediation server)");
                    ...
                    Program.Log($"[Mesh] Entering mesh-control-only loop — isIntroducer={isIntroducer}, natType={detectedNatType}, introducerMeshIP={introducerMeshIP ?? "null"}");
                    ...
                    while (!Program.ShutdownRequested && !Program.DisconnectRequested)
                    {
                        ...
                        if (reconnectedTcpClient != null && reconnectedTcpClient.Connected)
                        {
                                ...
                                if (reconnectedTcpBuffer.Length > 0)
                                {
                                    ...
                                    while (parsedMsg != null)
                                    {
                                            ...
                                            parsedMsg.ID == MediationMessageType.MeshPeerList)
                                        {
                                            ...
                                            {
                                                Program.Log($"[Mesh] Reconnect discovery: found {parsedMsg.Peers.Length} peer(s)");
                                                ...
                                                foreach (var peerObj2 in parsedMsg.Peers)
                                                {
                                                    ...
                                                    if (!string.IsNullOrEmpty(mip2))
                                                    {
                                                        ...
                                                        Program.Log($"[Mesh] Cached peer info: {mip2} = NAT:{(NATType)nt2}, endpoint:{ep2}");
                                                    }
                                                }
                                        ...
                                        {
                                            Program.Log($"[Mesh] Reconnect: received ConnectionBegin for connection {parsedMsg.ConnectionID}");
                                            ...
                                            if (!string.IsNullOrEmpty(parsedMsg.PrivateAddressString))
                                            {
                                                ...
                                                foreach (var oldConnID in oldConnIDs)
                                                {
                                                    ...
                                                    if (oldTunnel != null)
                                                    {
                                                        Program.Log($"[Mesh] Reconnect: disposing old tunnel {oldConnID} for {reconMeshIP} (superseded by {parsedMsg.ConnectionID})");
                                                        try { oldTunnel.Dispose(); } catch { }
                                                    }
                                                    peerMeshIPs.Remove(oldConnID);
                                                }
                                            ...
                                            if (!activeConnectionTunnels.ContainsKey(parsedMsg.ConnectionID))
                                            {
                                                ...
                                                var reconnectTunnel = new Tunnel(
                                                    ...
                                                    onConnectionComplete: () =>
                                                    {
                                                        Program.Log($"[Mesh] Reconnect tunnel {capturedConnID} WireGuard established");
                                                        System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
                                                        System.Threading.Interlocked.Increment(ref metricTunnelsEstablished);
                                                        lock (meshLock)
                                                        {
                                                ...
                                                System.Threading.Tasks.Task.Run(() =>
                                                {
                                                    ...
                                                    {
                                                        Program.Log($"[Mesh] Reconnect tunnel error: {ex.Message}");
                                                        System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
                                                    }
                                                });
                                            }
                                        ...
                                        {
                                            isIntroducer = true;
                                            Program.Log($"[Mesh] Reconnect: selected as introducer for {parsedMsg.PeerID}");
                                            ...
                                            if (parsedMsg.OtherPeers != null)
                                            {
                                                foreach (var peerObj in parsedMsg.OtherPeers)
                                                {
                                                    ...
                                                    {
                                                        string chosenRelay = PickRelay(exMeshIP, parsedMsg.PrivateAddressString) ?? (TunnelOptions.AllowRelayThrough ? meshIP : null);
                                                        if (string.IsNullOrEmpty(chosenRelay))
                                                        {
                                                            Program.Log($"[Mesh] No eligible relay for {exMeshIP} <-> {parsedMsg.PrivateAddressString} and self-relay disabled — skipping pair");
                                                            continue;
                                                        }
                                                        Program.Log($"[Mesh] Reconnect: both {parsedMsg.PeerID} and {exPeerID} are symmetric — relay via {(chosenRelay == meshIP ? "self" : chosenRelay)}");
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            {
                                                                Program.Log($"[Mesh] Failed to send MeshRelayAssignment to {chosenRelay}: {ex2.Message}");
                                                            }
                                                        }
                                                        ...
                                                        {
                                                            Program.Log($"[Mesh] Failed to send relay MeshConnectionBegin to {exMeshIP}: {ex2.Message}");
                                                        }
                                                        ...
                                                        if (!string.IsNullOrEmpty(parsedMsg.PrivateAddressString))
                                                        {
                                                            ...
                                                            if (completedTunnelMeshIPs.Contains(parsedMsg.PrivateAddressString))
                                                            {
                                                                ...
                                                                {
                                                                    Program.Log($"[Mesh] Failed to send relay MeshConnectionBegin to {parsedMsg.PrivateAddressString}: {ex2.Message}");
                                                                }
                                                            }
                                                            else
                                                            {
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        MeshSend(cbBytes, cbBytes.Length,
                                                            new IPEndPoint(IPAddress.Parse(exMeshIP), MeshControlPort));
                                                        Program.Log($"[Mesh] Reconnect introducer: sent MeshConnectionBegin to {exMeshIP} about {parsedMsg.PeerID}");
                                                    }
                                                    catch (Exception ex2)
                                                    {
                                                        Program.Log($"[Mesh] Failed to send MeshConnectionBegin to {exMeshIP}: {ex2.Message}");
                                                    }
                                                        ...
                                                        if (completedTunnelMeshIPs.Contains(parsedMsg.PrivateAddressString))
                                                        {
                                                            try
                                                            {
                                                                ...
                                                                MeshSend(cbNewBytes, cbNewBytes.Length,
                                                                    new IPEndPoint(IPAddress.Parse(parsedMsg.PrivateAddressString), MeshControlPort));
                                                                Program.Log($"[Mesh] Reconnect introducer: sent MeshConnectionBegin to {parsedMsg.PrivateAddressString} about {exPeerID}");
                                                            }
                                                            catch (Exception ex2)
                                                            {
                                                                Program.Log($"[Mesh] Failed to send MeshConnectionBegin to {parsedMsg.PrivateAddressString}: {ex2.Message}");
                                                            }
                                                        }
                                                        else
                                                        {
                                                            ...
                                                            Program.Log($"[Mesh] Reconnect introducer: deferred MeshConnectionBegin to {parsedMsg.PrivateAddressString} about {exPeerID}");
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    (DateTime.UtcNow - lastReconnectDiscovery.Value).TotalSeconds > reconnectDiscoverySeconds)
                                {
                                    ...
                                    if (stillIsolated && pendingTunnelCount == 0 && pendingConnectionRequests.Count == 0)
                                    {
                                        ...
                                        if (reconnectDiscoveryAttempts > MaxReconnectDiscoveryAttempts)
                                        {
                                            ...
                                            Program.Log($"[Mesh] {MaxReconnectDiscoveryAttempts} rediscovery attempts failed — tearing down reconnected connection to start fresh");
                                        ...
                                        else
                                        {
                                            ...
                                            Program.Log($"[Mesh] Re-sent discovery request ({reconnectDiscoveryAttempts}/{MaxReconnectDiscoveryAttempts}), next in {reconnectDiscoverySeconds}s");
                                        }
                                    }
                                    else if (!stillIsolated)
                                    {
                                        if (!isIntroducer)
                                        {
                                            ...
                                            Program.Log("[Mesh] Peers recovered — closing reconnected mediation connection");
                                            reconnectedTcpClient.Close();
                                            reconnectedTcpClient = null;
                                            reconnectedStream = null;
                                            reconnectedTcpBuffer = "";
                            ...
                            {
                                Program.Log($"[Mesh] Reconnected TCP error: {ex.Message}");
                                reconnectedTcpClient = null;
                                reconnectedStream = null;
                                reconnectedTcpBuffer = "";
                            }
                            ...
                            DateTime.UtcNow - lastIsolationCheck > isolationCheckInterval)
                        {
                            ...
                            if (!hasActivePeers && pendingTunnelCount == 0)
                            {
                                if (isolationDetectedAt == null)
                                {
                                    ...
                                    Program.Log($"[Mesh] Isolation detected — no active WireGuard peers. Will reconnect in {IsolationGracePeriodSeconds}s if not resolved.");
                                }
                                else if ((DateTime.UtcNow - isolationDetectedAt.Value).TotalSeconds >= IsolationGracePeriodSeconds)
                                {
                                    Program.Log("[Mesh] Isolation persisted — reconnecting to mediation server for peer discovery");
                                    try
                                    {
                                        ...
                                        if (TunnelOptions.TlsEnabled)
                                        {
                                            ...
                                            Program.Log($"[Mesh] Reconnect TLS handshake complete (protocol: {sslStream.SslProtocol})");
                                        }
                                        else
                                        {
                                        ...
                                        if (natTypeRespR.ID == MediationMessageType.NATTypeResponse)
                                        {
                                            ...
                                            Program.Log($"[Mesh] Reconnect NAT type: {detectedNatType}");
                                        }
                                        ...
                                        lastReconnectDiscovery = DateTime.UtcNow;
                                        Program.Log("[Mesh] Reconnected to mediation server — sent discovery request");
                                    }
                                    catch (Exception ex)
                                    {
                                        Program.Log($"[Mesh] Failed to reconnect to mediation: {ex.Message}");
                                        reconnectedTcpClient = null;
                                        reconnectedStream = null;
                            ...
                            else
                            {
                                if (isolationDetectedAt != null)
                                {
                                    Program.Log("[Mesh] Isolation resolved — active peers detected");
                                    isolationDetectedAt = null;
                                }
                            }
                        }
                        ...
                        if ((DateTime.UtcNow - lastRelayHealthCheck).TotalMilliseconds >= RelayHealthCheckIntervalMs)
                        {
                            ...
                            if (relayRoutes.Count > 0)
                            {
                                ...
                                {
                                    Program.Log($"[Mesh] Relay gateway(s) dead: {string.Join(", ", deadGateways)} — cleaning up stale routes");
                            ...
                            DateTime.UtcNow - lastIntroducerProbe > introducerProbeInterval)
                        {
                            ...
                            if (introducerMissedProbes >= IntroducerMissedProbeThreshold)
                            {
                                ...
                                string electionTarget = introducerMeshIP;
                                int delayMs = new Random().Next(0, 5000);
                                Program.Log($"[Mesh] Introducer confirmed dead — election delay {delayMs}ms before takeover attempt");
                                for (int slept = 0; slept < delayMs; slept += 100)
                                {
                                    System.Threading.Thread.Sleep(100);
                                    if (Program.ShutdownRequested || Program.DisconnectRequested) break;
                                    if (introducerMeshIP != electionTarget)
                                    {
                                        Program.Log("[Mesh] Election aborted — another peer became introducer during delay");
                                        introducerMissedProbes = 0;
                                        introducerProbeAckReceived = true;
                                        break;
                                    }
                                }
                                bool takeoverAborted = introducerMeshIP != electionTarget || Program.ShutdownRequested || Program.DisconnectRequested;
                                if (takeoverAborted)
                                {
                                ...
                                if (!takeoverAborted) try
                                    {
                                        Program.Log("[Mesh] Election delay elapsed — reconnecting to mediation to claim introducer role");
                                        var mediationEP = TunnelOptions.MediationEndpoint;
                                        reconnectedTcpClient = new TcpClient();
                                        reconnectedTcpClient.Client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true);
                                        reconnectedTcpClient.Connect(mediationEP);
                                        if (TunnelOptions.TlsEnabled)
                                        {
                                            ...
                                            sslStream.AuthenticateAsClient(mediationEP.Address.ToString());
                                            reconnectedStream = sslStream;
                                            Program.Log($"[Mesh] Takeover TLS handshake complete (protocol: {sslStream.SslProtocol})");
                                        }
                                        else
                                        {
                                            reconnectedStream = reconnectedTcpClient.GetStream();
                                            ...
                                            joinResp.IntroducerPeerID == peerID.ToString())
                                        {
                                            isIntroducer = true;
                                            introducerMeshIP = meshIP;
                                            Program.Log("[Mesh] Server confirmed us as new introducer");
                                        }
                                        else
                                        {
                                            Program.Log($"[Mesh] Server picked different introducer: {joinResp.IntroducerPeerID ?? "(none)"}");
                                            ...
                                            if (!string.IsNullOrEmpty(newIntroducerMeshIP))
                                            {
                                                ...
                                                if (newIntroducerMeshIP != oldIntroducerMeshIP)
                                                    Program.Log($"[Mesh] Introducer mesh IP updated: {oldIntroducerMeshIP ?? "(none)"} → {newIntroducerMeshIP}");
                                            }
                                            else
                                            {
                                                // Server chose someone we can't locate (or null). Clear our cached
                                                // introducer pointer so we don't keep probing a dead IP. Next
                                                // heartbeat with IsIntroducer=true will set it correctly.
                                                if (!string.IsNullOrEmpty(oldIntroducerMeshIP))
                                                    Program.Log($"[Mesh] Could not locate new introducer's mesh IP — clearing cached introducer pointer (was {oldIntroducerMeshIP})");
                                        ...
                                        if (string.IsNullOrEmpty(introducerMeshIP))
                                        {
                                            // Server returned (none) or we couldn't resolve the chosen introducer.
                                            // Keep the missed-probes counter at threshold so the next loop iteration
                                            // retries the takeover bid. ProbeInterval acts as natural backoff.
                                            introducerMissedProbes = IntroducerMissedProbeThreshold;
                                            introducerProbeAckReceived = false;
                                            Program.Log("[Mesh] No introducer resolved — will retry takeover on next probe cycle");
                                        }
                                        else
                                        {
                                            introducerMissedProbes = 0;
                                    ...
                                    {
                                        Program.Log($"[Mesh] Failed to reconnect for introducer takeover: {ex.Message}");
                        ...
                        if (!isIntroducer)
                        {
                            ...
                            {
                                Program.Log($"[Mesh] Peer {staleIP} has been silent for >{staleThreshold.TotalMinutes}m — removing locally");
                ...
                if (Program.DisconnectRequested && !Program.ShutdownRequested)
                {
                    Program.ConnectionState = MeshConnectionState.Disconnecting;
                    Program.Log("[Mesh] Disconnect requested — performing graceful leave");
                    ...
                    Program.ConnectionState = MeshConnectionState.Disconnected;
                    Program.Log("[Mesh] Disconnected — waiting for reconnect request");
                    ...
                    if (!Program.ShutdownRequested)
                    {
                        Program.Log("[Mesh] Reconnect requested — re-entering connect loop");
        ...
        {
            Program.Log($"[Mesh] Error: {ex.Message}");
        ...
        System.Threading.Tasks.Task.Run(async () =>
        {
            while (!Program.ShutdownRequested)
            {
                try
                {
                    ...
                    {
                        Program.Log($"[Mesh] Received MeshConnectionBegin from {result.RemoteEndPoint}: peer {controlMsg.PeerID} at {controlMsg.EndpointString}");
                        string senderIP = result.RemoteEndPoint.Address.ToString();
                        if (string.IsNullOrEmpty(introducerMeshIP) && senderIP != meshIP)
                        {
                            introducerMeshIP = senderIP;
                            Program.Log($"[Mesh] Learned introducer mesh IP from MeshConnectionBegin: {introducerMeshIP}");
                        }
                    ...
                    else if (controlMsg.ID == MediationMessageType.MeshHeartbeat)
                    {
                        ...
                        if (heartbeatSenderIP != meshIP)
                        {
                            ...
                            if (isIntroducer && wasCapable && !controlMsg.RelayCapable)
                            {
                                ...
                                Program.Log($"[Mesh] Relay {heartbeatSenderIP} dropped RelayCapable — queued reselection for affected pairs");
                            }
                        }
                        ...
                        {
                            Program.Log($"[Mesh] Introducer changed: {introducerMeshIP ?? "(none)"} → {heartbeatSenderIP} (heartbeat claim)");
                        ...
                        {
                            introducerMeshIP = heartbeatSenderIP;
                            Program.Log($"[Mesh] Learned introducer mesh IP from MeshHeartbeat: {introducerMeshIP}");
                        }
                        if (controlMsg.PeerRoster != null)
                        {
                                ...
                                heartbeatSenderIP == introducerMeshIP)
                            {
                                foreach (var knownIP in peerInfoByMeshIP.Keys.ToArray())
                                {
                                    ...
                                    Program.Log($"[Mesh] Synthesizing MeshPeerRemoved for {knownIP} — absent from introducer's roster");
                    ...
                    {
                        Program.Log($"[Mesh] Received MeshPeerRemoved: peer {controlMsg.PrivateAddressString} (peerID: {controlMsg.PeerID}) declared dead by introducer");
                        meshPeerRemovedQueue.Enqueue(controlMsg);
                    }
                    else if (controlMsg.ID == MediationMessageType.MeshPeerLeave)
                    {
                        Program.Log($"[Mesh] Received MeshPeerLeave: peer {controlMsg.PrivateAddressString} (peerID: {controlMsg.PeerID}) left gracefully");
                    ...
                    {
                        Program.Log($"[Mesh] Received MeshRelayAssignment: {controlMsg.PeerA} <-> {controlMsg.PeerB} via {controlMsg.RelayMeshIP}");
                        meshRelayAssignmentQueue.Enqueue(controlMsg);
                    }
                    else if (controlMsg.ID == MediationMessageType.MeshRelayHealthReport)
                    {
                        Program.Log($"[Mesh] Received MeshRelayHealthReport from {controlMsg.Self}: pair {controlMsg.PeerA}<->{controlMsg.PeerB} relay {controlMsg.CurrentRelay} obs={controlMsg.Observation}");
                ...
                {
                    Program.Log($"[Mesh] Mesh control listener error: {ex.Message}");
                }
            }
        });
    }
    ...
    private void ProcessMeshConnectionBegin(MediationMessage cbMsg)
    {
        ...
        {
            Program.Log($"[Mesh] MeshConnectionBegin missing PeerID — skipping");
            return;
        }
        // Relay mode only needs mesh IP + introducer IP, not endpoint
        if (!cbMsg.IsRelay && string.IsNullOrEmpty(remoteEndpoint))
        {
            Program.Log($"[Mesh] MeshConnectionBegin missing endpoint (non-relay) — skipping");
            return;
        }
        ...
        if (!cbMsg.IsRelay && pendingConnectionRequests.TryGetValue(remotePeerID, out var pendingTime))
        {
            ...
            {
                Program.Log($"[Mesh] Ignoring MeshConnectionBegin for {remotePeerID} — connection already pending ({(int)(DateTime.UtcNow - pendingTime).TotalSeconds}s ago)");
                return;
            }
            // Stale pending request — clean up and allow the new attempt
            Program.Log($"[Mesh] Clearing stale pending request for {remotePeerID} ({(int)(DateTime.UtcNow - pendingTime).TotalSeconds}s old) — allowing new attempt");
        ...
        if (cbMsg.IsRelay && wasRelayed && !string.IsNullOrEmpty(remoteMeshIP))
        {
                ...
                currentGateway.ToString() != newGateway)
            {
                ...
                if (wireguardTunnel.AddRelayRoute(IPAddress.Parse(newGateway), IPAddress.Parse(remoteMeshIP)))
                {
                    ...
                    Program.Log($"[Mesh] Relay reselect applied: {remoteMeshIP} now via {newGateway}");
                }
                else
                {
                    Program.Log($"[Mesh] Relay reselect: AddRelayRoute failed for {remoteMeshIP} via {newGateway}");
                }
                return;
            }
            if (routeExists)
            {
                Program.Log($"[Mesh] Ignoring duplicate relay MeshConnectionBegin for {remotePeerID} ({remoteMeshIP}) — relay route confirmed in WireGuard");
                return;
            }
            // Route is gone — clear stale tracking and let the message re-establish it
            Program.Log($"[Mesh] Relay route for {remoteMeshIP} missing from WireGuard — allowing re-establishment");
        ...
        if ((alreadyTracked || wasRelayed) && !string.IsNullOrEmpty(remoteMeshIP))
        {
            ...
            Program.Log($"[Mesh] Peer {remotePeerID} ({remoteMeshIP}) being re-introduced — cleaning up old connection (relay={wasRelayed})");
            metricReconnects++;
            RemoveDeadPeer(remoteMeshIP);
        }

        Program.Log($"[Mesh] Processing MeshConnectionBegin: peer {remotePeerID} at {remoteEndpoint} (NAT: {remotePeerNatType}, meshIP: {remoteMeshIP}, relay: {cbMsg.IsRelay})");
        ...
        if (cbMsg.IsRelay && !string.IsNullOrEmpty(remoteMeshIP))
        {
            ...
            if (!string.IsNullOrEmpty(gatewayIP))
            {
                ...
                {
                    Program.Log($"[Mesh] Failed to add relay route for {remoteMeshIP} via {gatewayIP}");
                }
            }
            else
            {
                Program.Log($"[Mesh] Relay MeshConnectionBegin missing RelayMeshIP/IntroducerMeshIP — cannot set up relay route");
            }
        ...
        var peerTunnel = new Tunnel(
            onConnectionFailure: () =>
            {
                Program.Log($"[Mesh] Introducer-relayed tunnel for {capturedPeerID} failed — cleaning up for future retry");
                lock (meshLock)
                {
            ...
            onConnectionComplete: () =>
            {
                Program.Log($"[Mesh] Introducer-relayed tunnel for {capturedPeerID} WireGuard established");
                System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
                System.Threading.Interlocked.Increment(ref metricTunnelsEstablished);
                lock (meshLock)
                {
                    ...
                    if (!string.IsNullOrEmpty(capturedMeshIP))
                    {
                        ...
                        {
                            Program.Log($"[Mesh] Flushing {deferred.Count} deferred MeshConnectionBegin message(s) for {capturedMeshIP}");
                            foreach (var deferredMsg in deferred)
                            {
                                string targetIP = !string.IsNullOrEmpty(deferredMsg.IntroducerMeshIP) && !deferredMsg.IsRelay
        ...
        System.Threading.Tasks.Task.Run(() =>
        {
            try
            {
                ...
                Program.Log($"[Mesh] Hole-punching started for {capturedPeerID} at {remoteEndpoint}");
            }
            catch (Exception ex)
            {
                Program.Log($"[Mesh] Error starting introducer-relayed tunnel for {capturedPeerID}: {ex.Message}");
                pendingConnectionRequests.Remove(capturedPeerID);
                System.Threading.Interlocked.Decrement(ref pendingTunnelCount);
            }
        });
    ...
    {
        Program.Log("[Mesh] Graceful shutdown initiated");
        try
        {
            var leaveMsg = new MediationMessage(MediationMessageType.MeshPeerLeave)
            {
            ...
            foreach (var peer in allPeers)
            {
                ...
                {
                    Program.Log($"[Mesh] Failed to send MeshPeerLeave to {peer.PrivateAddress}: {ex.Message}");
                }
            }
            Program.Log($"[Mesh] Sent MeshPeerLeave to {allPeers.Count()} peer(s)");
        }
        catch (Exception ex)
        {
            Program.Log($"[Mesh] Error sending graceful shutdown message: {ex.Message}");
        }
        Program.ShutdownRequested = true;
    }
    ...
    private void ProcessDiscoveredPeers(object[] peers, Stream targetStream = null)
    {
        ...
        Program.Log($"[Mesh] Discovered {peers.Length} peer(s) in network:");
    ...
    private void RemoveDeadPeer(string deadMeshIP)
    {
        ...
        Program.Log($"[Mesh] Removing dead peer {deadMeshIP} (peerID: {deadPeerID ?? "unknown"})");
        ...
        if (removedRelays.Count > 0)
        {
            ...
            Program.Log($"[Mesh] Removed {removedRelays.Count} relay route(s) via {deadMeshIP}");
        }
    ...
    {
        long LoadFactorMs = TunnelOptions.RelayLoadFactorMs;
        ...
        if (TunnelOptions.AllowRelayThrough && !string.IsNullOrEmpty(meshIP) && meshIP != a && meshIP != b)
        {
            int selfActive = HostedRelayCount();
            long? s = Score(meshIP, true, selfActive);
            if (s.HasValue) ranked.Add((meshIP, TunnelOptions.OwnRelayCapacity, s.Value));
        }
    ...
    private MeshState GetMeshState()
    {
        try
        {
            var state = new MeshState
            {
                ...
                ConnectionState = Program.ConnectionState.ToString(),
        ...
        {
            Program.Log($"[Mesh] Error building mesh state: {ex.Message}");
    ...
    private void NotifyEndpoint(string toIP, string remoteIP, string newRelay)
    {
        ...
        catch (Exception ex) { Program.Log($"[Mesh] Failed to notify {toIP} of reselect: {ex.Message}"); }
    }
                    ...
                    Stream mediationStream)
    {
            ...
            for (int j = i + 1; j < targetList.Count; j++)
            {
                ...
                if (!aReportsB || !bReportsA)
                {
                    ...
                    if (relayedPairs.Contains(pairKey))
                    {
                        ...
                        {
                            assignedRelay = PickRelay(ipA, ipB) ?? (TunnelOptions.AllowRelayThrough ? meshIP : null);
                            if (string.IsNullOrEmpty(assignedRelay))
                            {
                                Program.Log($"[Mesh] Heartbeat: relayed pair {ipA} <-> {ipB} broken but no eligible relay — skipping");
                                continue;
                            }
                        ...
                        Program.Log($"[Mesh] Heartbeat: relayed pair {ipA} <-> {ipB} broken — re-asserting via {(assignedRelay == meshIP ? "self" : assignedRelay)}");
                        ...
                        else
                        {
                            ...
                            catch (Exception ex) { Program.Log($"[Mesh] Failed to send MeshRelayAssignment to {assignedRelay}: {ex.Message}"); }
                        }
                        ...
                        if (!aReportsB)
                        {
                            ...
                            catch (Exception ex) { Program.Log($"[Mesh] Failed to send relay repair to {ipA}: {ex.Message}"); }
                        }
                        if (!bReportsA)
                        {
                            ...
                            catch (Exception ex) { Program.Log($"[Mesh] Failed to send relay repair to {ipB}: {ex.Message}"); }
                        }
                    ...
                    if (!hasA || !hasB)
                    {
                        Program.Log($"[Mesh] Heartbeat: missing peer info for pair {ipA}(known={hasA}) <-> {ipB}(known={hasB}) — peerInfoByMeshIP has {peerInfoByMeshIP.Count} entries");
                        continue;
                    }
                    ...
                    {
                        Program.Log($"[Mesh] Repair escalation ({attempts} attempts): {ipA} <-> {ipB} — requesting fresh NAT traversal via mediation");
                        if (mediationClient != null && mediationClient.Connected)
                        {
                            if (!string.IsNullOrEmpty(infoA.peerID) && !pendingConnectionRequests.ContainsKey(infoA.peerID))
                            {
                                ...
                                {
                                    Program.Log($"[Mesh] Failed to send escalation ConnectionRequest for {ipA}: {ex.Message}");
                                }
                            }
                            if (!string.IsNullOrEmpty(infoB.peerID) && !pendingConnectionRequests.ContainsKey(infoB.peerID))
                            {
                                ...
                                {
                                    Program.Log($"[Mesh] Failed to send escalation ConnectionRequest for {ipB}: {ex.Message}");
                                }
                            }
                        }
                        lastRepairAttempt[pairKey] = DateTime.UtcNow;
                    ...
                    else if (bothSymmetric)
                    {
                        ...
                        {
                            chosenRelay = PickRelay(ipA, ipB) ?? (TunnelOptions.AllowRelayThrough ? meshIP : null);
                            if (string.IsNullOrEmpty(chosenRelay))
                            {
                                Program.Log($"[Mesh] No eligible relay for {ipA} <-> {ipB} and self-relay disabled — skipping");
                                continue;
                            }
                        ...
                        Program.Log($"[Mesh] Heartbeat: {ipA} <-> {ipB} disconnected (both symmetric) — re-establishing relay via {(chosenRelay == meshIP ? "self" : chosenRelay)} (attempt {attempts})");
                        ...
                        {
                            Program.Log($"[Mesh] Failed to send relay repair to {ipA}: {ex.Message}");
                        }
                        ...
                        {
                            Program.Log($"[Mesh] Failed to send relay repair to {ipB}: {ex.Message}");
                        }
                    ...
                    else
                    {
                        ...
                        Program.Log($"[Mesh] Heartbeat: {ipA} <-> {ipB} disconnected — re-introducing (attempt {attempts}{(sameLanPair ? ", same-LAN" : "")})");
                        ...
                        if (!string.IsNullOrEmpty(endpointForA))
                        {
                            ...
                            {
                                Program.Log($"[Mesh] Failed to send repair MeshConnectionBegin to {ipA}: {ex.Message}");
                            }
                        }
                        ...
                        if (!string.IsNullOrEmpty(endpointForB))
                        {
                            ...
                            {
                                Program.Log($"[Mesh] Failed to send repair MeshConnectionBegin to {ipB}: {ex.Message}");
                            }
                        }
                        lastRepairAttempt[pairKey] = DateTime.UtcNow;
                    }
        ...
        foreach (var ip in targetList)
        {
            ...
            {
                Program.Log($"[Mesh] Heartbeat: peer {ip} cannot reach introducer ({meshIP}) — requesting re-connection via mediation");
                if (mediationClient != null && mediationClient.Connected &&
                    ...
                    !pendingConnectionRequests.ContainsKey(lostPeerInfo.peerID))
                {
                    ...
                    {
                        Program.Log($"[Mesh] Failed to send re-connection request for {ip}: {ex.Message}");
                    }
                }
            }
        }
